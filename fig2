<x-layout title="Dashboard" heading="My Medication Reminders">
    ...
    <div id="reminderModal"
        ...
        <div
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-2">💊 Time for your medication!</h2>
            <p id="reminderText" class="text-gray-600 mb-6">It’s time to take your pills.</p>
            ...
            <p class="text-xs text-gray-400 mt-3">Stay consistent — your health matters ❤️</p>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", async function () {
            ...
            // Add Stop Alarm button dynamically
            let stopBtn = document.getElementById("stopAlarm");
            if (!stopBtn) {
                stopBtn = document.createElement("button");
                stopBtn.id = "stopAlarm";
                stopBtn.className = "bg-red-600 text-white font-semibold px-6 py-2 rounded-full shadow-md hover:bg-red-700 transition-all duration-300 ml-2";
                stopBtn.textContent = "Stop Alarm";
                dismiss.parentNode.insertBefore(stopBtn, dismiss.nextSibling);
            }
            ...
            function playAlarm() {
                stopAlarm(); // Ensure no overlapping
            ...
            function triggerReminder(reminder) {
                if (activeReminders.has(reminder.id) || alarmDisabled) return;

                // Only trigger if time is still due (within 2 minutes window)
                const now = new Date();
                const reminderTime = new Date(reminder.time);
                if (Math.abs(now - reminderTime) / 1000 > 120) return;
                ...
                if (Notification.permission === "granted") {
                    new Notification("💊 Time to take your medication", {
                        body: reminder.name,
                        icon: "/images/pills.jpg"
                    });
                }

                // Show modal
                text.textContent = `It’s time to take your ${reminder.name}.`;
                ...
                stopBtn.onclick = () => {
                    stopAlarm();
                    alarmDisabled = true;
                    // Optionally hide modal immediately
                    modal.classList.add("hidden");
                    modal.classList.remove("flex");
                    clearInterval(interval);
                    activeReminders.delete(reminder.id);
            ...
            function checkReminders() {
                if (alarmDisabled) return; // Don't check if alarm is disabled for this session
